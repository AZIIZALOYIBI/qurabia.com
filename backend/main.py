...
# قاعدة بيانات مصغرة للجذور العربية (Python backend — مستوحى من pysarf)
_ARABIC_ROOTS: Dict[str, Dict[str, Any]] = {
...
_ARABIC_PREFIXES = ['وال', 'بال', 'كال', 'فال', 'لل', 'ال', 'و', 'ف', 'ب', 'ك', 'ل', 'س']
_ARABIC_SUFFIXES = ['ات', 'ون', 'ين', 'ان', 'هم', 'هن', 'كم', 'نا', 'ها', 'ية', 'ة', 'ي', 'ه']
_ARABIC_PARTICLES = {'في', 'من', 'الى', 'على', 'عن', 'مع', 'بين', 'حتى', 'هل', 'ما', 'لا', 'لم',
                     'قد', 'ان', 'هذا', 'هذه', 'ذلك', 'الذي', 'التي', 'هو', 'هي', 'نحن', 'كل'}
_CONSONANTS = set('بتثجحخدذرزسشصضطظعغفقكلمنهء')
...
def _extract_root_backend(word: str) -> Dict[str, Any]:
    """استخراج الجذر من كلمة عربية"""
    normalized = _normalize_arabic(word)

    # البحث المباشر في المشتقات
    for root_key, entry in _ARABIC_ROOTS.items():
        normalized_root = _normalize_arabic(root_key)
        for deriv in entry['derivatives']:
            if _normalize_arabic(deriv) == normalized:
    ...
    for root_key, entry in _ARABIC_ROOTS.items():
        nr = _normalize_arabic(root_key)
        if nr == stem:
            return {'root': nr, 'confidence': 0.9, 'entry': entry}

    # استخراج الحروف الصامتة
    consonants = [c for c in stem if c in _CONSONANTS]
    if len(consonants) >= 3:
        candidate = consonants[0] + consonants[1] + consonants[2]
        for root_key, entry in _ARABIC_ROOTS.items():
            nr = _normalize_arabic(root_key)
            root_cons = [c for c in nr if c in _CONSONANTS]
