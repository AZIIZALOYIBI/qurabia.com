...
export class GeminiService {
  ...
  static async analyzeSimulation(results: Record<string, unknown> | object): Promise<string> {
    try {
      ...
      if (!apiBase) return GeminiService.generateMockAnalysis(results);

      const response = await fetch(`${apiBase}/api/llm/gemini/analyze`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ results: results ?? {} }),
      });
      if (!response.ok) return GeminiService.generateMockAnalysis(results);
      const data = (await response.json()) as GeminiAnalysisResponse;
      const text = (data?.text ?? '').toString();
      return text.trim() ? text.trim() : GeminiService.generateMockAnalysis(results);
    } catch {
      return GeminiService.generateMockAnalysis(results);
    }
  }
