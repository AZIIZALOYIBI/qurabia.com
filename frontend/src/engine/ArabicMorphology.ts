...
const ROOTS_DB: Record<string, RootEntry> = {
  ...
  كتب: {
    ...
    field: 'knowledge',
    derivatives: ['كتاب', 'كاتب', 'مكتوب', 'مكتبة', 'كتابة', 'مكاتبة'],
  },
  علم: {
    meaning: 'العلم والمعرفة',
    field: 'knowledge',
    derivatives: ['عالم', 'معلوم', 'علوم', 'تعليم', 'معلم', 'عليم'],
  },
  ...
  // ─── الإدراك والحواس ───
  نظر: {
    meaning: 'النظر والمشاهدة',
    field: 'perception',
    derivatives: ['ناظر', 'منظور', 'نظر', 'نظرة', 'نظرية', 'منظار'],
  },
  ...
  عرف: {
    meaning: 'المعرفة والتعرف',
    field: 'knowledge',
    derivatives: ['عارف', 'معروف', 'عرفان', 'معرفة', 'تعريف', 'عرف'],
  },
...
/** حروف العلة */
const _VOWEL_LETTERS = new Set('اويى');
...
function identifyPattern(word: string, root: string): { pattern: string; patternName: string; morphMeaning: string } {
  ...
  const [_f, _ain, _lam] = rootLetters;
...
export function analyzeWord(word: string): MorphAnalysis {
  ...
  // تحديد الوزن
  const { pattern, patternName, morphMeaning: _morphMeaning } = identifyPattern(cleaned, root);
  ...
  const rootLetters: [string, string, string] = [letters[0] || '؟', letters[1] || '؟', letters[2] || '؟'];
