...
const prob0 = (sv: StateVector1Q) => complexAbs(sv[0]) ** 2;
const prob1 = (sv: StateVector1Q) => complexAbs(sv[1]) ** 2;
...
describe('GATE_H (Hadamard)', () => {
  ...
  it('H|0⟩ → |+⟩ (equal superposition)', () => {
    const state: StateVector1Q = [{ real: 1, imag: 0 }, { real: 0, imag: 0 }];
  ...
  it('H|1⟩ → |-⟩ (equal superposition)', () => {
    const state: StateVector1Q = [{ real: 0, imag: 0 }, { real: 1, imag: 0 }];
    const result = applyGate1Q(GATE_H, state);
    expect(complexAbs(result[0])).toBeCloseTo(SQRT2_INV, 10);
    expect(complexAbs(result[1])).toBeCloseTo(SQRT2_INV, 10);
  });
...
describe('gateRZ', () => {
  ...
  it('preserves norm for |+⟩ state', () => {
    const state: StateVector1Q = [{ real: SQRT2_INV, imag: 0 }, { real: SQRT2_INV, imag: 0 }];
