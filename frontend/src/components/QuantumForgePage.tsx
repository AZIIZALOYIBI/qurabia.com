...
import { type DecisionResult, analyzeDecision } from '../engine/GroverDecision';
import { type ForgeResult, forgeText } from '../engine/QuantumForge';
import {
  type CircuitGate,
  type SemanticCircuit,
  buildSemanticCircuit,
  circuitToASCII,
} from '../engine/QuantumSemanticCircuit';
...
const CopyButton: React.FC<{
  ...
  <button
    ...
    style={{
      background: 'none',
      border: 'none',
      color: copiedField === field ? 'var(--p-success)' : 'var(--fg-3)',
      cursor: 'pointer',
      padding: 4,
      flexShrink: 0,
    }}
...
const StatCard: React.FC<{ label: string; value: string | number; color: string; icon: React.ElementType }> = ({
  label,
  value,
  color,
  icon: Icon,
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <InputArea
        ...
        onChange={setInput}
        onSubmit={run}
        placeholder="اكتب جملة عربية... مثال: العلم نور والجهل ظلام"
        buttonLabel="كمّم"
      />
      ...
      {analysis && circuit && (
        ...
        >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 10 }}>
            ...
            <StatCard
              label="التشابك"
              value={`${(circuit.entanglementDegree * 100).toFixed(0)}%`}
              color="#A78BFA"
              icon={Lock}
            />
          </div>
                ...
                .map((w, i) => (
                  <div
                    ...
                    style={{
                      ...
                      gap: 6,
                      minWidth: 120,
                      animation: `uiPopIn var(--dur-3) var(--ease-snap) ${i * 50}ms both`,
                    }}
                  >
                    ...
                    <div
                      style={{ fontSize: 11, color: 'var(--fg-3)', fontFamily: 'var(--font-ar)', textAlign: 'center' }}
                    >
                      {w.patternName || w.pattern}
                    </div>
                    <div
                      style={{ fontSize: 10, color: 'var(--fg-3)', fontFamily: 'var(--font-ar)', textAlign: 'center' }}
                    >
                      {w.rootMeaning}
                    </div>
                    ...
                    {w.superpositionStates.length > 1 && (
                      ...
                      >
                        |ψ⟩ ={' '}
                        {w.superpositionStates.slice(0, 4).map((s, k) => (
                          <span key={k}>
                            {k > 0 ? ' + ' : ''}
                            {s}
                          </span>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <InputArea
        ...
        onChange={setInput}
        onSubmit={run}
        placeholder="اكتب رسالة لتشفيرها... مثال: السلام عليكم"
        buttonLabel="شفّر"
      />
      ...
      {result && (
        ...
        >
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 10 }}>
            ...
            <StatCard
              label="التعقيد"
              value={`${result.complexityScore.toFixed(0)}%`}
          ...
          <div className="ui-card" style={{ padding: 18, borderRadius: 16 }}>
            ...
            <div style={{ display: 'grid', gap: 12 }}>
              <div>
                <div style={{ fontSize: 11, color: 'var(--fg-3)', marginBottom: 4, fontFamily: 'var(--font-ar)' }}>
                  النص المشفّر
                </div>
                <div
                  style={{
                ...
                >
                  ...
                  <CopyButton
                    text={result.encryption.cipherText}
                    field="cipher"
          ...
          <div className="ui-card" style={{ padding: 18, borderRadius: 16 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <code
                  style={{ fontFamily: 'var(--font-mono)', fontSize: 14, color: 'var(--p-primary)', letterSpacing: 1 }}
                >
            ...
            <div style={{ display: 'flex', gap: 16, marginTop: 10, flexWrap: 'wrap' }}>
              ...
              <MetricChip
                label="الدقة"
                value={`${(result.fingerprint.fidelity * 100).toFixed(1)}%`}
                color="var(--p-primary)"
              />
              <MetricChip
                label="التماسك"
                value={`${(result.fingerprint.coherenceScore * 100).toFixed(1)}%`}
                color="var(--p-tertiary)"
              />
            </div>
          </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <InputArea
        ...
        onChange={setInput}
        onSubmit={run}
        placeholder="اكتب سؤال قرارك... مثال: هل أفتح مشروع مطعم أم متجر إلكتروني؟"
        buttonLabel="حلّل"
      />
      ...
      {result && (
        ...
        >
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 10 }}>
            ...
            <StatCard
              label="الوضوح"
              value={`${(result.decisionClarity * 100).toFixed(0)}%`}
              color="#A78BFA"
              icon={BrainCircuit}
            />
          </div>
            ...
            {result.options.map((opt, i) => (
              ...
              >
                ...
                <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                  <MetricChip
                    label="الدلالة"
                    value={`${(opt.semanticWeight * 100).toFixed(0)}%`}
                    color="var(--p-secondary)"
                  />
                  <MetricChip
                    label="الإيجابية"
                    value={`${(opt.positivityScore * 100).toFixed(0)}%`}
                    color="var(--p-success)"
                  />
                  <MetricChip
                    label="العملية"
                    value={`${(opt.practicalityScore * 100).toFixed(0)}%`}
                    color="var(--p-tertiary)"
                  />
                  <MetricChip
                    label="الثقة"
                    value={`${(opt.confidenceScore * 100).toFixed(0)}%`}
                    color="var(--p-primary)"
                  />
                </div>
                ...
                {opt.dominantFields.length > 0 && (
                  <div style={{ display: 'flex', gap: 6, marginTop: 10, flexWrap: 'wrap' }}>
                    {opt.dominantFields.map((f, j) => (
                      <FieldBadge key={j} field={f} />
                    ))}
                  </div>
                )}
              </div>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <InputArea
        ...
        onChange={setInput}
        onSubmit={run}
        placeholder="اكتب نصاً لتوليد بصمته الكمومية..."
        buttonLabel="ولّد"
      />
      ...
      {result && visualGrid && (
        ...
        >
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <code
                style={{ fontFamily: 'var(--font-mono)', fontSize: 16, color: 'var(--p-primary)', letterSpacing: 2 }}
              >
                {result.fingerprint.hash}
              </code>
          ...
          >
            <StatCard
              label="الإنتروبيا"
              value={result.fingerprint.entropy.toFixed(3)}
              color="var(--p-secondary)"
              icon={Sparkles}
            />
            <StatCard
          ...
          >
            هذه البصمة فريدة لهذا النص — أي تغيير ولو بحرف واحد سينتج بصمة مختلفة تماماً. يمكن استخدامها للتحقق من أصالة
            المحتوى العربي وحماية حقوق النشر.
          </div>
        </div>
      )}
    </div>
