import { AlertTriangle, ArrowRight, Database, Play, RefreshCw, Search, Zap } from 'lucide-react';
...
export const GroverSearchModule: React.FC = () => {
  ...
  return (
    ...
    >
      <div
        className="absolute top-0 right-0 w-64 h-64 bg-yellow-500/5 rounded-full blur-3xl -mr-20 -mt-20 pointer-events-none"
        aria-hidden="true"
      />

      <div className="mb-6 flex justify-between items-start z-10">
        <div>
          <h2 className="text-xl font-semibold text-white mb-1">محرك البحث الكمومي (Grover)</h2>
          <p className="text-sm text-slate-400 font-mono">O(√N) Unstructured Database Search</p>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div className="p-3 bg-yellow-500/5 border border-yellow-500/20 rounded-lg space-y-2">
              <div className="flex justify-between text-xs font-mono">
                ...
                <span className="text-red-400">{Math.floor(dbSize / 2)} خطوة (متوسط)</span>
              </div>
              <div className="flex justify-between text-xs font-mono">
                <span className="text-slate-400">البحث الكمومي O(√N):</span>
                <span className="text-emerald-400">{optimalSteps} خطوات (أمثل)</span>
              </div>
            </div>
          </div>
        ...
        <div className="md:col-span-2 bg-black/40 rounded-lg border border-white/5 p-4 flex flex-col">
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-2 text-yellow-400">
              <Database size={18} />
              <h3 className="font-mono text-sm uppercase tracking-wider">تضخيم السعة (Amplitude Amplification)</h3>
            </div>
            <div className="flex gap-4 text-xs font-mono" aria-live="polite">
              ...
              <div className="text-slate-400">
                احتمال الهدف:{' '}
                <span className={currentProb > 50 ? 'text-emerald-400 font-bold' : 'text-yellow-400'}>
          ...
          <div
            className="flex-grow min-h-[250px] w-full"
            role="img"
            aria-label={`مخطط احتمالات البحث الكمومي — احتمال الهدف: ${currentProb.toFixed(2)}%`}
          >
          ...
          {currentStep >= optimalSteps && optimalSteps > 0 && (
            <div
              className="mt-4 p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-lg flex items-center gap-3 text-emerald-400 animate-pulse"
              role="status"
            >
              <Zap size={20} />
              <span className="text-sm font-medium">
                تم العثور على العنصر بنجاح! احتمال القياس: {currentProb.toFixed(2)}% في {currentStep} خطوات فقط.
              </span>
            </div>
          )}
        </div>
