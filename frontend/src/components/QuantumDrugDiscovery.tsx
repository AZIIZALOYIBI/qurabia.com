...
class VQEEngine {
  ...
  _estimateEnergy(theta: number[]) {
    ...
    for (let i = 0; i < theta.length; i++) {
      ...
      _contribution += term.coeff * Math.cos(theta[i]) * Math.cos(theta[i]);
    }
...
export const QuantumDrugDiscovery: React.FC = () => {
  ...
  const runVQE = useCallback(() => {
    ...
    const interval = setInterval(() => {
      ...
      currentTheta = currentTheta.map((t) => t - 0.01 * (Math.random() - 0.5));
      const currentEnergy = engine._estimateEnergy(currentTheta);
      setEnergy(currentEnergy);

      if (currentProgress % 20 === 0) {
        setLogs((prev) => [...prev, 'تحديث المعلمات... الدقة الحالية: 10^-4 Hartree']);
      }

      if (currentProgress >= 100) {
        clearInterval(interval);
        setRunning(false);
        setLogs((prev) => [...prev, '✅ اكتمل التحسين. تم العثور على طاقة الحالة القاعية بدقة 10^-4 Hartree.']);
      }
    }, 200);
  }, [running]);
  ...
  return (
    <div className="p-6 bg-transparent h-full flex flex-col relative overflow-hidden">
      <div className="flex items-center justify-between mb-6 z-10">
        <div>
          <h2 className="text-xl font-semibold text-white mb-1">اكتشاف الأدوية الكمومي</h2>
          <p className="text-sm text-slate-400 font-mono">VQE Engine (Molecular Simulation)</p>
        </div>
        <div
          className={`p-3 rounded-full ${
            running ? 'bg-purple-500/20 text-purple-400 animate-pulse' : 'bg-slate-800 text-slate-400'
          }`}
        >
          <Activity size={24} />
        </div>
      ...
      <div className="flex-grow flex flex-col justify-center space-y-6">
        <div className="bg-black/40 rounded-lg p-4 border border-white/5 font-mono text-sm h-48 overflow-y-auto flex flex-col gap-2">
          {logs.length === 0 ? (
            <span className="text-slate-600 italic">في انتظار بدء المحاكاة...</span>
          ) : (
            logs.map((log, i) => (
              <div key={i} className="text-emerald-400/80 flex items-start gap-2">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-slate-800/50 p-4 rounded-lg border border-white/5">
            <div className="text-xs font-mono text-slate-400 uppercase tracking-wider mb-1">طاقة الحالة القاعية</div>
            <div className="text-xl font-light text-cyan-400 font-mono">
              {energy !== null ? `${energy.toFixed(4)} Ha` : '---'}
            </div>
            <div className="text-[10px] text-slate-500 mt-1 font-mono">Hartree</div>
          </div>
          <div className="bg-slate-800/50 p-4 rounded-lg border border-white/5">
            <div className="text-xs font-mono text-slate-400 uppercase tracking-wider mb-1">الدقة الكيميائية</div>
